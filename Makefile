...
.PHONY: clean # clean artifacts generated by make tests
clean:
	-rm -rf test.yml .venv/ nodiscc-xsrv-*.tar.gz gitea-cli/ .venv/ ansible_collections/ .cache/

.PHONY: install_deps # install requirements for test suite
install_deps:
...
.PHONY: check_readmes # check that all roles README.md contains expected sections/info
check_readmes:
...
.PHONY: check_jinja2 # Jinja2 template syntax linter
check_jinja2: venv
	source .venv/bin/activate && \
...
.PHONY: update_todo # Update TODO.md by fetching issues from the main gitea instance API (requires gitea-cli configuration in ~/.config/gitearc)
update_todo:
...
.PHONY: changelog # establish a changelog since the last git tag
changelog:
...
.PHONY: bump_versions # bump version numbers in repository files (new_tag=X.Y.Z required)
bump_versions:
ifndef new_tag
...
.PHONY: doc # run documentation generation tasks
...
.PHONY: doc_html # HTML documentation generation (sphinx-build --help)
doc_html:
