...
venv:
	...
	pip3 install wheel && \
	pip3 install isort ansible-lint==6.8.1 yamllint ansible==6.4.0
...
test_ansible_lint: venv
	source .venv/bin/activate && \
	ansible-lint -v -x fqcn[action-core],fqcn[action],deprecated-module,truthy,braces,line-length,name[casing],yaml[truthy],schema[meta],yaml[line-length],name[template] roles/*
