...
venv:
	...
	pip3 install wheel && \
	pip3 install isort ansible-lint==6.13.1 yamllint ansible==7.2.0
...
test_ansible_lint: venv
	source .venv/bin/activate && \
	ansible-lint -v -x fqcn[action-core],fqcn[action],truthy,braces,line-length,name[casing],yaml[truthy],schema[meta],yaml[line-length],name[template],args[module],yaml[octal-values] roles/*
...
clean:
	-rm -rf .venv/ nodiscc-xsrv-*.tar.gz gitea-cli/ .venv/ ansible_collections/ .cache/ tests/playbooks/xsrv-test/ansible_collections/ tests/playbooks/xsrv-test/.venv/ tests/playbooks/xsrv-test/data/cache/facts/ tests/playbooks/xsrv-test/data/certificates/ tests/playbooks/xsrv-test/data/public_keys
