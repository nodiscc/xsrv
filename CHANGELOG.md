...
- add netdata-modtime plugin and related configuration variables (files to watch, alarms)
- add a check for successful daily backups using netdata-modtime
...
- install apt-listchanges alongside unattended-upgrades (mail with a summary of changes will be sent to the server admin)
...
- allow installation from deb packages/packagecloud APT repository, make it the default
...
- upgrade netdata to 1.24.0 when installed from binary
- rsyslog: monitor samba log files with imfile module
- various fixes, reorder, cleanup, update documentation, fix role idempotence
