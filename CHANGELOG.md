...
- refactor documentation (clarify/cleanup/update/reorder/reword/simplify/deduplicate/move)
- import all roles from separate repositories (monorepo, will be published as ansible collection)
- fix automated tests (ansible-lint, yamllint), add ansible-playbook --syntax-check
...
- fix shellcheck warnings, fix quoting
- improve logging
...
- auto-restart services when required by default
- firewall: allow HTTP/HTTPS access from anywhere (required for let's encrypt http-01 challenge)
- firewall: allow access to all services from LAN by default
- firewall: change the default policy for the 'global' firehol_network definition to RETURN
...
- add playbook, inventory, host_vars (cleartext and vaulted) files
...
- fix fail2ban dbpurgeage configuration


**monitoring: refactor role:**
- import from https://gitlab.com/nodiscc/ansible-xsrv-monitoring
- add https://gitlab.com/nodiscc/netdata-logcount netdata module
...
- fix tt_rss_full_url templating (install directly at the root of the virtualhost)
...
- cleanup, remove obsolete handler, reorder tasks
...
- update documentation
- simplify domain name/install directory/full URL templatization
