...
Other collections:
- [nodiscc.toolbox](https://gitlab.com/nodiscc/toolbox/-/tree/master/ARCHIVE/ANSIBLE-COLLECTION) - less-maintained, experimental or project-specific roles (`awesome_selfhosted_html`, `bitmagnet`, `docker`, `homepage_extra_icons`, `icecast`, `k8s`, `mariadb`, `nfs_server`, `ollama`, `planarally`, `proxmox`, `pulseaudio`, `reverse_ssh_tunnel`, `rocketchat`, `rss2email`, `rss_bridge`, `valheim_server`, `vscodium`, `znc`)
